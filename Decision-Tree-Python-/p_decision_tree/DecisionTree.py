...
#         print(dot.source)
    ...
    def predict(self, node, sample):
        ...
        for child in node.childs:
            if sample[self.attributes.index(node.value)] == child.value:
                return self.predict(child.next, sample)
        return None
    def calculate_accuracy(self, test_samples, test_labels):
        correct_predictions = 0
        for sample, label in zip(test_samples, test_labels):
            prediction = self.predict(self.root, sample)
            if prediction == label:
